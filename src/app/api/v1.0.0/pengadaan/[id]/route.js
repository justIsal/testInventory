import {deletePengadaan, getPengadaanID, updatePengadaan} from "../services";
...
export async function PUT(request, {params}) {
 try {
  ...
  const data = await getPengadaanID(params.id);
  if (data.status) {
   const userUpdate = await updatePengadaan(params.id, data.message, body);
...
export async function DELETE(request, {params}) {
 try {
  const data = await getPengadaanID(params.id);
  if (data.status) {
   const userDelete = await deletePengadaan(params.id, data.message);
