import {deleteUser, getUserID, updateUser} from "../services";
...
export async function PUT(request, {params}) {
 try {
  ...
  const data = await getUserID(params.id);
  if (data.status) {
   const userUpdate = await updateUser(params.id, data.message, body);
...
export async function DELETE(request, {params}) {
 try {
  const data = await getUserID(params.id);
  if (data.status) {
   const userDelete = await deleteUser(params.id, data.message);
